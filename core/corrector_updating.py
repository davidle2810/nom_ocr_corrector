...
def correct(sn: str, qn: list[str]) -> list[str]:
    ...
    def get_similar(s, q, type = 0):
        S1 = [s] + sn_sim.get(s, list())
        if type == 0:
            S2 = qn_to_sn.get(q, list())
        else:
            S2 = qn_to_sn_without_accent.get(removing_accent(q), list())
        S2 = set(S2)
        S = [s for s in S1 if s in S2]
        if len(S) == 0: 
            if type == 0:
                return get_similar(s, q, type = 1)
            else:
                return None
        return S[0]
    ...
    def traceback(i, j):
        ...
        if i < n and j < m and similar[i][j] is not None and res == memoi(i + 1, j + 1):
            ...
            traceback(i + 1, j + 1)
        elif i < n and j < m  and res == memoi(i + 1, j + 1) + edit_cost['replace']:
            if similar[i][j] is None:
                r='X'
            else:
                r=similar[i][j]
            corrections.append(f'replace:{sn[j]}->{r}')
            traceback(i + 1, j + 1)
